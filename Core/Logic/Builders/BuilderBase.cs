...
public abstract class BuilderBase(Options options, ILogger logger)
{
    ...
    protected virtual async Task SplitBuild(Book book, string fileName)
    {
    ...
    {
        return $"{GetTitle(book)}.{Extension}";
    }
    ...
    {
        return BookNameBuilder.Build(Options.BookNamePattern, book).Crop(100);
    }
    ...
    public async Task Build(Book book)
    {
        ...
        if (book.SupportSplitting && (Options.SplitChapters || Options.SplitVolumes)) fileName = GetTitle(book);

        if (!string.IsNullOrWhiteSpace(Options.SavePath))
        {
            if (!Directory.Exists(Options.SavePath)) Directory.CreateDirectory(Options.SavePath);
        ...
        if (Options.Cover) await SaveCover(Options.SavePath, book.Cover, GetTitle(book));
