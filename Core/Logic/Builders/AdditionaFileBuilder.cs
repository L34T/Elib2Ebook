        ...
        if (options.HasAdditionalType(AdditionalTypeEnum.Images))
        {
            ...
            foreach (var chapter in book.Chapters ?? [])
            {
                ...
                var subPath = Path.Combine(additionalPath, AdditionalTypeEnum.Images.ToString(),
                    chapter.Title.RemoveInvalidChars());
                ...
                {
                    var fileName = Path.Combine(subPath,
                        $"{++c}{Path.GetExtension(image.FullName)}".RemoveInvalidChars());
            ...
            if (book.Cover is not null)
            {
                ...
                var fileName = Path.Combine(subPath,
                    $"Cover{Path.GetExtension(book.Cover.FullName)}".RemoveInvalidChars());
