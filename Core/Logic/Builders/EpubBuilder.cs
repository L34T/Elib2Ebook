...
public class EpubBuilder(Options options, ILogger logger) : BuilderBase(options, logger)
{
    ...
    private List<TempFile> Images { get; } = new();
    ...
    {
        return pattern.Replace("{title}", title.CleanInvalidXmlChars().ReplaceNewLine())
            .Replace("{body}", decodeText.HtmlDecode().CleanInvalidXmlChars()).AsXHtmlDoc().AsString();
    }
    ...
    private void WithFiles(string directory, string searchPattern)
    {
        foreach (var file in FileProvider.Instance.GetFiles(directory, searchPattern))
        {
            ...
            var type = file.Name.EndsWith(".ttf") ? EpubContentType.FontTruetype :
                file.Name.EndsWith(".css") ? EpubContentType.Css :
                throw new Elib2EbookFormatException($"Неизвестный тип файла {fileName}");
    ...
    {
        if (path.EndsWith(".jpg", StringComparison.InvariantCultureIgnoreCase)) return ImageFormat.Jpeg;

        if (path.EndsWith(".gif", StringComparison.InvariantCultureIgnoreCase)) return ImageFormat.Gif;

        if (path.EndsWith(".png", StringComparison.InvariantCultureIgnoreCase)) return ImageFormat.Png;
    ...
    private void Write(Book book)
    {
        ...
        foreach (var coAuthor in book.CoAuthors) _writer.AddAuthor(coAuthor.Name);

        _writer.SetTitle(book.Title);

        if (book.Cover != null) _writer.SetCover(book.Cover.Content, GetImageFormat(book.Cover.FullName));
        ...
        if (book.Seria is not null)
        {
            ...
            var seriaUrl = book.Seria.Url?.ToString();
            if (!string.IsNullOrWhiteSpace(seriaUrl) && seriaUrl.Length > 14)
                EpubSharpCompat.TrySetSeriesUrlIfSupported(_writer, seriaUrl.CleanInvalidXmlChars(), Logger);
        }
        ...
        foreach (var chapter in book.Chapters.Where(c => c.IsValid))
        {
            ...
            {
                _writer.AddFile(image.FullName, Array.Empty<byte>(),
                    GetImageFormat(image.FullName).ToEpubContentType());
                Images.Add(image);
            }

            _writer.AddChapter(chapter.Title.HtmlDecode().ReplaceNewLine().RemoveInvalidChars(),
                ApplyPattern(pattern, chapter.Title, chapter.Content));
        }

        WithFiles(Options.ResourcesPath, "*.ttf");
        WithFiles(Options.ResourcesPath, "*.css");

        var ncxWarningXhtml = FileProvider.Instance.ReadAllText($"{Options.ResourcesPath}/NcxWarning.xhtml");
        EpubSharpCompat.TryAddNcxWarningPageIfSupported(_writer, "Предупреждение", ncxWarningXhtml, Logger);
    }
