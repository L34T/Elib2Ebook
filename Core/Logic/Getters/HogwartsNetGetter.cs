...
public class HogwartsNetGetter(BookGetterConfig config) : GetterBase(config)
{
    ...
    public override async Task Authorize()
    {
        ...
        var doc = await Config.Client.PostHtmlDocWithTriesAsync("https://hogwartsnet.ru/mfanf/member.php".AsUri(),
            new FormUrlEncodedContent(payload), _encoding);
        var anketa = doc.QuerySelector("tr.top_fanf a[href*=anketa.php]");

        if (anketa is null)
            throw new Elib2EbookAuthException("Не удалось авторизоваться. Проверьте правильность ID/пароля");
    ...
    private async Task<HtmlDocument> GetChapter(Uri url)
    {
        ...
        if (doc.ParsedText.Contains(mature))
            throw new Elib2EbookParseException($"{mature}. Добавьте авторизацию по ID и паролю.");

        var text = new StringBuilder();
        using var sr = new StringReader(doc.QuerySelector("#chap_text").RemoveNodes("center, div[id*=yandex]").InnerText
            .HtmlDecode());
        while (true)
        {
