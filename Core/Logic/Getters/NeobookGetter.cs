...
{
    private readonly Uri _apiUrl = new("https://api.neobook.org/");
    ...
    {
        return new Author(data.User.LastName + " " + data.User.FirstName,
            SystemUrl.MakeRelativeUri(data.User.UserName));
    }
    ...
    {
        var doc = await Config.Client.GetHtmlDocWithTriesAsync(SystemUrl.MakeRelativeUri("reader/")
            .AppendQueryParameter("book", bookId).AppendQueryParameter("chapter", neobookTocChapter.Token));
        ...
        if (neobookBook.ActiveChapterIndex > neobookBook.Chapters.Length) return default;

        var chapter = neobookBook.Chapters[neobookBook.ActiveChapterIndex];
        if (string.IsNullOrWhiteSpace(chapter.Data?.Html)) return default;
