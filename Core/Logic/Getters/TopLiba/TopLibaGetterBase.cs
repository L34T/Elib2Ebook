...
public abstract class TopLibaGetterBase(BookGetterConfig config) : GetterBase(config)
{
    ...
    public override async Task Authorize()
    {
        ...
        if (helpBlock is null)
            Config.Logger.LogInformation("Успешно авторизовались");
        else
            throw new Elib2EbookAuthException($"Не удалось авторизоваться. {helpBlock.GetText()}");
    }
    ...
    {
        var data = await Config.Client.PostWithTriesAsync(SystemUrl.MakeRelativeUri($"/reader/{bookId}/chapter"),
            GetData(id, token));
    ...
    private async Task<IEnumerable<string>> GetToc(string bookId)
    {
        ...
        var result = new Regex("capters: \\[(?<chapters>.*?)\\]").Match(doc.Text).Groups["chapters"].Value
