...
public class DreameGetter(BookGetterConfig config) : GetterBase(config)
{
    ...
    {
        return url.GetSegment(2).Split(".")[0].Split("-")[0];
    }
    ...
    private async Task<HtmlDocument> GetChapter(DreameChapter urlChapter)
    {
        var response = await Config.Client.GetFromJsonWithTriesAsync<DreameApiResponse<DreameChapter>>(
            _apiUrl.MakeRelativeUri(
                $"/novel/getChapter?channel=dreamepmian-173&product=1&osType=2&cid={urlChapter.Id}&systemFlag=android&port=web"));
