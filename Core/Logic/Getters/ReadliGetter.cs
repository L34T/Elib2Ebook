...
public class ReadliGetter(BookGetterConfig config) : GetterBase(config)
{
    ...
    private static Author GetAuthor(HtmlDocument doc, Uri url)
    {
        ...
        return new Author(author.InnerText.HtmlDecode(),
            url.MakeRelativeUri(author.Attributes["href"]?.Value ?? string.Empty));
    }

    private static Seria GetSeria(HtmlDocument doc, Uri url)
    {
        var a = doc.QuerySelector("div.book-info a.book-info__link[href^=/serie/]");
        if (string.IsNullOrWhiteSpace(a.GetText())) return default;
    ...
    private static string GetBookId(Uri uri)
    {
        return uri.GetQueryParameter("b") ??
               throw new InvalidOperationException("Не удалось определить идентификатор книги");
    }
    ...
    private async Task<Uri> GetMainUrl(Uri url)
    {
        if (GetId(url).StartsWith("chitat-online", StringComparison.InvariantCultureIgnoreCase))
        {
            var doc = await Config.Client.GetHtmlDocWithTriesAsync(SystemUrl
                .MakeRelativeUri($"/chitat-online/?b={GetBookId(url)}").ReplaceHost(_host.Host));
    ...
    private Task<TempFile> GetCover(HtmlNode doc, Uri bookUri)
    {
        ...
        return !string.IsNullOrWhiteSpace(imagePath)
            ? SaveImage(bookUri.MakeRelativeUri(imagePath).ReplaceHost(_host.Host))
    ...
    private async Task AddChapter(ICollection<Chapter> chapters, Chapter chapter, StringBuilder text)
    {
        ...
        chapter.Images =
            await GetImages(chapterDoc, SystemUrl.MakeRelativeUri("/chitat-online/").ReplaceHost(_host.Host));
    ...
    private async Task<HtmlDocument> GetChapter(string bookId, int page)
    {
        var response = await Config.Client.GetWithTriesAsync(SystemUrl
            .MakeRelativeUri($"/chitat-online/?b={bookId}&pg={page}").ReplaceHost(_host.Host));
