...
public class BooksYandexBooksGetter(BookGetterConfig config) : BooksYandexGetterBase(config)
{
    ...
    protected override async Task<IEnumerable<Chapter>> FillChapters(Book book, BooksYandexResponse response)
    {
        ...
        if (response.Book?.SourceType == "serial") return await GetFromSerial(book, id);
        ...
        {
            var epubFile = await TempFile.Create(requestUri, Config.TempFolder.Path,
                epubResponse.Content.Headers.ContentDisposition.FileName.Trim('\"'),
                await epubResponse.Content.ReadAsStreamAsync());

            if (epubFile is not null && Config.Options.HasAdditionalType(AdditionalTypeEnum.Books))
    ...
    private async Task<IEnumerable<Chapter>> GetFromSerial(Book book, string id)
    {
        ...
        foreach (var episode in episodes.Where(e => e.CanBeRead))
        {
            ...
            var epubResponse = await Config.Client.GetAsync(requestUri);
            if (epubResponse.StatusCode != HttpStatusCode.OK) continue;
    ...
    private async Task<IEnumerable<BooksYandexEpisode>> GetEpisodes(string id)
    {
        try
        {
            ...
            for (var i = 1;; i++)
            {
                var response = await Config.Client.GetFromJsonAsync<BooksYandexEpisodes>(
                    $"https://api.bookmate.ru/api/v5/books/{id}/episodes".AsUri().AppendQueryParameter("page", i));
                if (response.Episodes.Length == 0) break;
        ...
        catch (HttpRequestException ex)
        {
            if (ex.StatusCode == HttpStatusCode.Unauthorized)
                throw new Elib2EbookAuthException("Авторизационный токен невалиден. Требуется обновление");
    ...
    private async Task<List<TempFile>> GetAudio(BooksYandexResponse bookResponse)
    {
        ...
        if (playlist.Tracks.Length == 0) return result;
        ...
        for (var i = 0; i < playlist.Tracks.Length; i++)
        {
            ...
            result.Add(await TempFile.Create(url.AsUri(), Config.TempFolder.Path, $"{i}_{url.AsUri().GetFileName()}",
    ...
    private async Task<BooksYandexPlaylist> GetPlayList(string id)
    {
        try
        {
            return await Config.Client.GetFromJsonAsync<BooksYandexPlaylist>(
                $"https://api.bookmate.ru/api/v5/audiobooks/{id}/playlists.json".AsUri());
        }
        catch (HttpRequestException ex)
        {
            if (ex.StatusCode == HttpStatusCode.Unauthorized)
