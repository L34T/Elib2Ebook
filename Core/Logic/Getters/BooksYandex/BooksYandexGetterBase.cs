...
public abstract class BooksYandexGetterBase(BookGetterConfig config) : GetterBase(config)
{
    ...
    {
        return base.IsSameUrl(url) && Paths.Any(path =>
            string.Equals(url.GetSegment(1), path, StringComparison.InvariantCultureIgnoreCase));
    }
    ...
    public override async Task<Book> Get(Uri url)
    {
        ...
        var details = (BookmateBookBase)bookResponse.Book ??
                      (BookmateBookBase)bookResponse.AudioBook ?? bookResponse.Comicbook;
    ...
    private async Task<BooksYandexResponse> GetBookResponse(string path, string id)
    {
        try
        {
            ...
            {
                var linkedResponse = await Config.Client.GetAsync(
                    $"https://api.bookmate.ru/api/v5/books/{booksYandexResponse.AudioBook.LinkedBooks[0]}".AsUri());
    ...
    private Author GetAuthor(BookmateBookBase book)
    {
        ...
        return author is null
            ? new Author("BooksYandex")
            : new Author(author.Name, SystemUrl.MakeRelativeUri($"/authors/{author.Uuid}"));
    }
}
