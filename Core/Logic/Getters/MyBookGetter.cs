...
public class MyBookGetter(BookGetterConfig config) : GetterBase(config)
{
    ...
    private void SetAuthHeader(string method, Uri uri)
    {
        ...
        _apiClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("OAuth",
            client.GetAuthorizationHeader().Replace("OAuth", string.Empty).Trim());
    }
    ...
    public override async Task Authorize()
    {
        ...
        else
        {
            ...
            throw new Elib2EbookAuthException($"Не удалось авторизоваться. {message}");
        }
    ...
    private async Task<MyBookBook> GetDetails(Uri url)
    {
        ...
        if (details.Type == "audio")
        {
            ...
            if (bookId.HasValue) return await GetDetailsApi(bookId.Value);
        }
    ...
    public override async Task<Book> Get(Uri url)
    {
        ...
        if (details.Type != "audio")
        {
            ...
            if (response.StatusCode != HttpStatusCode.OK)
                throw new Elib2EbookParseException("Не удалось получить книгу");

            var origBook = await TempFile.Create(bookUrl, Config.TempFolder.Path, bookUrl.GetFileName(),
        ...
        {
            if (details.Type == "audio") book.AdditionalFiles.Add(AdditionalTypeEnum.Audio, await GetAudio(details.Id));
    ...
    private async Task<List<TempFile>> GetAudio(long bookId)
    {
        ...
        var files = details.Files.Where(node => node is JsonObject).Select(node => node.Deserialize<MyBookFile>())
            .ToList();
        ...
        for (var i = 0; i < files.Count; i++)
        {
            ...
            result.Add(await TempFile.Create(url, Config.TempFolder.Path, $"{file.Order}_{name}{ext}",
                await response.Content.ReadAsStreamAsync()));
    ...
    private Task<TempFile> GetCover(MyBookBook book)
    {
        return !string.IsNullOrWhiteSpace(book.Cover)
            ? SaveImage($"https://i3.mybook.io/p/x378/{book.Cover.TrimStart('/')}".AsUri())
            : Task.FromResult(default(TempFile));
    }

    private Author GetAuthor(MyBookBook book)
    {
        if (book.Authors.Length == 0) return new Author("MyBook");

        var author = book.Authors[0];
        return string.IsNullOrWhiteSpace(author.Url)
            ? new Author(author.Name)
            : new Author(author.Name, SystemUrl.MakeRelativeUri(author.Url));
    }
