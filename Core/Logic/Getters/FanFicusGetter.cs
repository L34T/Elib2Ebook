...
public class FanFicusGetter(BookGetterConfig config) : GetterBase(config)
{
    ...
    public override async Task Authorize()
    {
        ...
        var response =
            await Config.Client.PostAsJsonAsync(_apiHost.MakeRelativeUri("api/v1/user/login"), GenerateAuthData());
        if (response.StatusCode == HttpStatusCode.OK)
        {
            ...
            Config.Client.DefaultRequestHeaders.Authorization =
                new AuthenticationHeaderValue("Bearer", user.Value.Token);
            Config.Logger.LogInformation("Успешно авторизовались");
        }
        else
        {
            throw new Elib2EbookAuthException("Не удалось авторизоваться. Неверный пароль или имейл");
        }
    }
    ...
    public override async Task<Book> Get(Uri url)
    {
        ...
        var response =
            await Config.Client.GetFromJsonWithTriesAsync<FanFicusApiResponse<FanFicusBook>>(
                _apiHost.MakeRelativeUri($"api/v1/post/{bookId}"));
    ...
    private Author GetAuthor(FanFicusBook book)
    {
        ...
        return creator is null
            ? new Author("FanFicus")
            : new Author(creator.NickName, SystemUrl.MakeRelativeUri($"/user/{creator.Id}"));
    }

    private async Task<IEnumerable<FanFicusPart>> GetToc(string bookId)
    {
        var response =
            await Config.Client.GetFromJsonWithTriesAsync<FanFicusApiResponse<FanFicusPart[]>>(
                _apiHost.MakeRelativeUri($"/api/v1/post/{bookId}/post-part"));
    ...
    {
        var response =
            await Config.Client.GetFromJsonWithTriesAsync<FanFicusApiResponse<FanFicusChapter>>(
                _apiHost.MakeRelativeUri($"api/v1/post/{bookId}/post-part/{fanFicusPart.Id}"));
