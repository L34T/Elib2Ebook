...
public class JaomixGetter(BookGetterConfig config) : GetterBase(config)
{
    ...
    private async Task<HtmlDocument> GetChapter(Uri url)
    {
        ...
        while (doc.QuerySelector("div.themeform div.h-captcha, div.themeform div.but-captcha") != null)
        {
            Config.Logger.LogInformation(
                $"Обнаружена капча. Перейдите по ссылке {url}, введите капчу и нажмите Enter...");
            Console.Read();
        ...
        foreach (var node in doc.QuerySelector("div.themeform").ChildNodes)
            if (node.Name != "br" && node.Name != "script" && !string.IsNullOrWhiteSpace(node.InnerHtml) &&
                node.Attributes["class"]?.Value?.Contains("adblock-service") == null)
            {
    ...
    {
        return doc.QuerySelectorAll("form.download-chapter .hiddenstab .flex-dow-txt a").Select(a =>
            new UrlChapter(url.MakeRelativeUri(a.Attributes["href"].Value), a.InnerText.Trim())).Reverse();
    }
}
