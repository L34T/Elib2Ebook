...
public abstract class RenovelsGetterBase(BookGetterConfig config) : GetterBase(config)
{
    ...
    public override async Task Authorize()
    {
        ...
        var response =
            await Config.Client.PostAsJsonAsync("https://api.renovels.org/api/users/login/".AsUri(), payload);
    ...
    {
        if (content.Branches.Length == 0) return new Author("Renovels");

        if (content.Branches[0].Publishers.Length == 0) return new Author("Renovels");
    ...
    {
        var response =
            await Config.Client.GetFromJsonAsync<RenovelsContent>(_apiUrl.MakeRelativeUri($"/api/v2/titles/{bookId}/"));
        return response;
    }
    ...
    private async Task<IEnumerable<RenovelsChapter>> GetToc(RenovelsContent content)
    {
        ...
        for (var i = 1;; i++)
        {
            var uri = _apiUrl.MakeRelativeUri(
                $"/api/v2/titles/chapters/?branch_id={content.Branches[0].Id}&ordering=-index&user_data=1&count=40&page={i}");
