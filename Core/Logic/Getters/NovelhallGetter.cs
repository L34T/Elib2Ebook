...
public class NovelhallGetter(BookGetterConfig config) : GetterBase(config)
{
    ...
    {
        var chapters = doc.QuerySelectorAll("#morelist ul li a[href]")
            .Select(a => new UrlChapter(url.MakeRelativeUri(a.Attributes["href"].Value), a.GetText())).ToList();
    ...
    private static Author GetAuthor(HtmlDocument doc)
    {
        ...
        return a is not null ? new Author(a.Attributes["content"].Value) : new Author("NovelHall");
    }
}
