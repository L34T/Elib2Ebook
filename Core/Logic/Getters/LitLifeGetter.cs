...
public class LitLifeGetter(BookGetterConfig config) : GetterBase(config)
{
    ...
    public override async Task Authorize()
    {
        ...
        doc = await Config.Client.PostHtmlDocWithTriesAsync(SystemUrl.MakeRelativeUri("/login"),
            GenerateAuthData(token));
        var alert = doc.QuerySelector("div.alert-danger");
        if (alert is null)
            Config.Logger.LogInformation("Успешно авторизовались");
        else
            throw new Elib2EbookAuthException($"Не удалось авторизоваться. {alert.GetText()}");
    }
    ...
    public override async Task<Book> Get(Uri url)
    {
        ...
        if (noAccess is not null) throw new Elib2EbookParseException("Книна не доступна для чтения.");
    ...
    private async Task<IEnumerable<UrlChapter>> GetToc(string id)
    {
        var doc = await Config.Client.GetHtmlDocWithTriesAsync(
            SystemUrl.MakeRelativeUri($"/books/{id}/sections/list_go_to"));

        var result = doc.QuerySelectorAll("ul a")
            .Select(a => new UrlChapter(SystemUrl.MakeRelativeUri(a.Attributes["href"].Value),
                a.RemoveNodes("span").GetText().ReplaceNewLine()))
            .ToList();
