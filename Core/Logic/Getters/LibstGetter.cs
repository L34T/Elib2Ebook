...
public class LibstGetter(BookGetterConfig config) : GetterBase(config)
{
    ...
    public override async Task Authorize()
    {
        ...
        doc = await Config.Client.PostHtmlDocWithTriesAsync(SystemUrl.MakeRelativeUri("/Account/Login?ReturnUrl=/"),
        ...
        if (string.IsNullOrEmpty(error))
            ...
            throw new Elib2EbookAuthException($"Не удалось авторизоваться. {error}");
    }
    ...
    private async Task<IEnumerable<Chapter>> FillChapters(HtmlDocument doc)
    {
        ...
        foreach (var bookChapter in GetToc(doc))
        {
            ...
            if (chapterDoc is not null) chapter.Content = chapterDoc.DocumentNode.InnerHtml;
    ...
    {
        if (string.IsNullOrWhiteSpace(bookChapter.Id)) return default;

        var sb = new StringBuilder();

        for (var i = 1;; i++)
        {
            var response = await Config.Client.GetAsync(
                SystemUrl.MakeRelativeUri($"/Detail/GetChapterTextForCanvas?ChapID={bookChapter.Id}&k=0&page={i}"));
