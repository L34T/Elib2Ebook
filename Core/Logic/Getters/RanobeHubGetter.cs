...
public class RanobeHubGetter(BookGetterConfig config) : GetterBase(config)
{
    ...
    private async Task<HtmlDocument> GetChapter(string url)
    {
        ...
        var result = doc.QuerySelector("div.container[data-container]")
            .RemoveNodes("div.title-wrapper, div.ads-desktop, div.tablet, div.chapter-hoticons").InnerHtml.AsHtmlDoc();
    ...
    private async Task<IEnumerable<RanobeHubChapter>> GetToc(HtmlDocument doc)
    {
        ...
        var response =
            await Config.Client.GetFromJsonAsync<RanobeHubApiResponse>(
                SystemUrl.MakeRelativeUri($"/api/ranobe/{internalId}/contents"));
