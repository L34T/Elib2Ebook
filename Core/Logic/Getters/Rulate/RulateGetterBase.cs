...
public abstract class RulateGetterBase(BookGetterConfig config) : GetterBase(config)
{
    ...
    {
        return url.Segments.Length == 3 ? base.GetId(url) : url.GetSegment(2);
    }
    ...
    public override async Task Authorize()
    {
        ...
        if (alertBlock is null)
        {
            ...
            {
                // Ignore deserialization errors if the response is not valid JSON
            }

            if (!string.IsNullOrWhiteSpace(error?.Error))
                throw new Elib2EbookAuthException($"Не удалось авторизоваться. {error.Error}");

            Config.Logger.LogInformation("Успешно авторизовались");
        }
        else
        {
            throw new Elib2EbookAuthException($"Не удалось авторизоваться. {alertBlock.GetText()}");
        }
    }
    ...
    {
        var s = await Config.Client.GetFromJsonWithTriesAsync<RulateChapter>(
            SystemUrl.MakeRelativeUri($"/book/{bookId}/{chapterId}/readyajax"));
        return (s.CanRead
            ? s.Content.AsHtmlDoc().QuerySelector("div.content-text")?.InnerHtml ?? string.Empty
            : string.Empty).AsHtmlDoc();
    }
