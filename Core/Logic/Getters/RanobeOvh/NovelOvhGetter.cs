...
public class NovelOvhGetter(BookGetterConfig config) : RanobeOvhGetterBase(config)
{
    ...
    private StringBuilder AsHtml(RanobeOvhChapterFull chapterFullResponse,
        IEnumerable<RanobeOvhChapterContent> contents)
    {
    ...
    {
        var data = await Config.Client.GetFromJsonAsync<RanobeOvhChapterFull>(
            $"https://api.{SystemUrl.Host}/v2/chapters/{ranobeOvhChapterFull.Id}");
