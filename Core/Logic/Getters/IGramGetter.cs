...
public class IGramGetter(BookGetterConfig config) : GetterBase(config)
{
    ...
    public override async Task Authorize()
    {
        ...
        using var post =
            await Config.Client.PostAsync(SystemUrl.MakeRelativeUri("/account/login/"), GenerateAuthData(token));
        var checkLogin =
            await Config.Client.GetFromJsonAsync<LitexitUser>(SystemUrl.MakeRelativeUri("/api/v2/users/current/"));
        if (checkLogin?.Id > 0)
    ...
    {
        var response =
            await Config.Client.GetAsync(SystemUrl.MakeRelativeUri($"/api/v1/book/{GetInternalId(doc)}/chapters"));
