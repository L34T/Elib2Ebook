...
public class BookriverGetter(BookGetterConfig config) : GetterBase(config)
{
    ...
    {
        var response =
            await Config.Client.SendAsync(
                GetMessage(_apiUrl.MakeRelativeUri($"/api/v1/books/chapter/text/{bookChapterId}")));
        if (response.StatusCode == HttpStatusCode.Forbidden) return default;
    ...
    {
        var response =
            await Config.Client.GetWithTriesAsync(
                _apiUrl.MakeRelativeUri($"/api/v1/books/chapters/text/published?bookId={bookId}"));
        var content = await response.Content.ReadAsStringAsync();
