...
public abstract class NovelxoGetterBase(BookGetterConfig config) : GetterBase(config)
{
    ...
    {
        if (url.Segments.Length == 2) return SystemUrl.MakeRelativeUri(GetId(url));
            ...
            foreach (var node in container.QuerySelectorAll("p, div.ctp"))
                ...
                else if (node.Name == "div" && node.Attributes["class"]?.Value?.Contains("ctp") == true)
                {
                    if (node.Attributes["class"].Value.Contains("protected"))
                        ...
                        sb.Append(Encoding.UTF8.GetString(Transform(Convert.FromBase64String(node.InnerText), Key, IV))
                            .HtmlDecode());
                }
    ...
    private async Task<string> GetProtected(HtmlDocument doc, HtmlNode node)
    {
        ...
        var headerName = Regex.Match(doc.ParsedText, $"{header.Groups["name"].Value} = \'(?<id>.*?)\'").Groups["id"]
            .Value;
        ...
        var url = new Uri(
            $"https://a.novelxo.com/v1/chapters/{id}/ctp?width=1140&sign=932870{Atob(Convert.ToBase64String(encode))}182906");
        ...
        var result = Encoding.UTF8.GetString(Transform(Convert.FromBase64String(readAsStringAsync.Trim('\"')), Key, IV))
            .HtmlDecode();
