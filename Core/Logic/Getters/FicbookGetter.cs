...
public class FicbookGetter(BookGetterConfig config) : GetterBase(config)
{
    ...
    public override async Task Init()
    {
        ...
        Config.Client.DefaultRequestHeaders.TryAddWithoutValidation("User-Agent",
    ...
    private IEnumerable<UrlChapter> GetToc(HtmlDocument doc, Uri url, string title)
    {
        ...
        if (links.Count == 0)
            ...
            foreach (var li in links)
            {
                ...
                if (a != null)
                    result.Add(new UrlChapter(url.MakeRelativeUri(a.Attributes["href"].Value),
                        li.GetTextBySelector("h3")));
            }
