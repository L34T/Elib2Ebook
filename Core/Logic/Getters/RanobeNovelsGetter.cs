...
public class RanobeNovelsGetter(BookGetterConfig config) : GetterBase(config)
{
    ...
    private async Task<List<UrlChapter>> GetToc(HtmlDocument doc)
    {
        ...
        foreach (var span in doc.QuerySelectorAll("ul.myUL span.caret[data-id]"))
        {
            ...
            var post = await Config.Client.PostWithTriesAsync(
                SystemUrl.MakeRelativeUri("/wp-content/themes/ranobe-novels/template-parts/post/menu-query.php"),
                payload);
            var toc = await post.Content.ReadFromJsonAsync<WuxiaWorldToc[]>();
            result.AddRange(toc.Select((a, i) =>
                new UrlChapter(SystemUrl.MakeRelativeUri(a.PostName), $"Глава {offset + i + 1}")));
        }
    ...
    private async Task<HtmlDocument> GetSafety(Uri url)
    {
        ...
        while (response is null || response.StatusCode == HttpStatusCode.ServiceUnavailable)
        {
