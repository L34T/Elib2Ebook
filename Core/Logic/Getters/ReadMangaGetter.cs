...
public class ReadMangaGetter(BookGetterConfig config) : GetterBase(config)
{
    ...
    private async Task<HtmlDocument> GetChapter(UrlChapter urlChapter)
    {
        ...
        if (doc.QuerySelector("div.buy-button") is not null) return default;

        var json = Regex.Match(doc.ParsedText, @"readerInit(.*?)(?<data>\[\[(.*?)]]),").Groups["data"].Value
            .Replace("'", "\"").Deserialize<List<JsonElement[]>>();
        ...
        foreach (var elem in json)
        {
            ...
            if (imageUri.Host == "one-way.work") imageUri = imageUri.MakeRelativeUri(imageUri.AbsolutePath);
