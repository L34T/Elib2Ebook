...
public abstract class NewLibSocialGetterBase(BookGetterConfig config) : GetterBase(config)
{
    ...
    private static string Challenge(string str)
    {
        ...
        {
            sb.Append(ALPHABET_CHALLENGE[bytes[t] >> 2]);
            sb.Append(ALPHABET_CHALLENGE[((bytes[t] & 3) << 4) | (bytes[t + 1] >> 4)]);
            sb.Append(ALPHABET_CHALLENGE[
                ((bytes[t + 1] & 15) << 2) | ((t + 2 < bytes.Length ? bytes[t + 2] : 0) >> 6)]);
            sb.Append(ALPHABET_CHALLENGE[(t + 2 < bytes.Length ? bytes[t + 2] : 0) & 63]);
        }
    ...
    {
        if (string.IsNullOrEmpty(Config.Options.Token) && !Config.HasCredentials) return;
        ...
        if (!string.IsNullOrEmpty(Config.Options.Token))
        {
            ...
            Config.Client.DefaultRequestHeaders.Authorization =
                new AuthenticationHeaderValue("Bearer", Config.Options.Token);
            return;
        }
    ...
    private async Task<HtmlNode> SubmitCredentialsAsync(Uri challengeUrl, string csrfToken)
    {
        ...
        await Config.Client.PostHtmlDocWithTriesAsync(AuthHost.MakeRelativeUri("/auth/login"),
    ...
    private async Task<string> ProcessAuthorizationConsentAsync(HtmlNode postForm)
    {
        ...
        using var authorize = await Config.Client.PostAsync(
            AuthHost.MakeRelativeUri(postForm.Attributes["action"].Value), new FormUrlEncodedContent(payload));
        if ((int)authorize.StatusCode == 419)
            throw new Elib2EbookAuthException(
                "Авторизация отклонена (код 419). Обычно это означает истекший CSRF токен или необходимость пройти капчу. Попробуйте указать --flare");
        if (authorize is null || authorize.RequestMessage?.RequestUri is null)
            throw new Elib2EbookAuthException("Сайт не вернул код авторизации (проверьте логин/пароль)");
    ...
    private async Task ExchangeTokenAsync(Uri redirectUri, string secret, string authCode)
    {
        ...
        if (tokenResponse is null)
            throw new Elib2EbookAuthException("Не удалось получить ответ от сайта при запросе access token");
        ...
        if (tokenResponse.StatusCode != HttpStatusCode.OK)
        {
            ...
            throw new Elib2EbookAuthException(
                $"Сайт не выдал access token. Код {(int)tokenResponse.StatusCode} ({tokenResponse.StatusCode}).{snippet}");
        }
    ...
    private async Task<RanobeLibBookDetails> GetBookDetails(Uri url)
    {
        ...
        if (response is null)
            throw new Elib2EbookParseException("Не удалось получить ответ от сайта при загрузке информации о книге");

        if (response.StatusCode != HttpStatusCode.OK)
            throw new Elib2EbookParseException(
                $"Ошибка загрузки информации о книге. Код {(int)response.StatusCode} ({response.StatusCode}).");
    ...
    private async Task<IEnumerable<SocialLibBookChapter>> GetToc(RanobeLibBookDetails book, string bid)
    {
        ...
        var response = await Config.Client.GetWithTriesAsync(url);
        if (response is null)
            throw new Elib2EbookParseException("Не удалось получить ответ от сайта при загрузке оглавления");

        if (response.StatusCode != HttpStatusCode.OK)
            throw new Elib2EbookParseException(
                $"Ошибка загрузки оглавления. Код {(int)response.StatusCode} ({response.StatusCode}).");

        var result = await response.Content.ReadFromJsonAsync<SocialLibBookChapters>()
            .ContinueWith(t => t.Result.Chapters);
    ...
    private Author GetAuthor(RanobeLibBookDetails details)
    {
        ...
        return author is null
            ? new Author("Ranobelib")
            : new Author(author.Name, SystemUrl.MakeRelativeUri($"/ru/people/{author.SlugUrl}"));
    }
    ...
    private IEnumerable<Author> GetCoAuthors(RanobeLibBookDetails details)
    {
        return details.Data.Authors
            .Skip(1)
            .Select(author => new Author(author.Name, SystemUrl.MakeRelativeUri($"/ru/people/{author.SlugUrl}")))
            .ToList();
    }

    private Task<TempFile> GetCover(RanobeLibBookDetails details)
    {
        return !string.IsNullOrWhiteSpace(details.Data.Cover.Default)
            ? SaveImage(details.Data.Cover.Default.AsUri())
    ...
    {
        var uri = ApiHost.MakeRelativeUri(
            $"/api/manga/{book.Data.SlugUrl}/chapter?number={chapter.Number}&volume={chapter.Volume}");
        if (!string.IsNullOrWhiteSpace(bid)) uri = uri.AppendQueryParameter("branch_id", bid);

        var chapterResponse =
            await Config.Client.GetFromJsonWithTriesAsync<SocialLibBookChapterResponse>(uri, TimeSpan.FromSeconds(30));
