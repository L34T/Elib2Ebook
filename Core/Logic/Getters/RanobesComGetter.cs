...
public class RanobesComGetter(BookGetterConfig config) : GetterBase(config)
{
    ...
    private async Task Antibot(HtmlDocument doc, Uri referrer)
    {
        ...
        var post = await Config.Client.PostAsync(SystemUrl.MakeRelativeUri("antibot8/ab.php"),
    ...
    private async Task<HtmlDocument> GetChapter(UrlChapter chapter)
    {
        ...
        foreach (var node in doc.QuerySelectorAll("#arrticle > :not(.splitnewsnavigation)"))
        {
            ...
            {
                if (node.InnerHtml?.Contains("window.yaContextCb") == false) sb.Append(node.InnerHtml.CoverTag(tag));
            }
        }
    ...
    {
        var relativeUri = doc.QuerySelector("div.r-fullstory-btns a[title~=оглавление]").Attributes["href"].Value
            .HtmlDecode();
    ...
    private async Task<IEnumerable<UrlChapter>> GetToc(Uri tocUri)
    {
        ...
        for (var i = 1; i <= pages; i++)
        {
            ...
            var chapters = doc
                ...
                .Select(a => new UrlChapter(a.Attributes["href"].Value.AsUri(),
                    string.IsNullOrWhiteSpace(a.Attributes["title"].Value)
                        ? "Без названия"
