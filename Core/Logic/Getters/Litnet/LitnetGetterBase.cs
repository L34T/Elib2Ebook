...
public abstract class LitnetGetterBase(BookGetterConfig config) : GetterBase(config)
{
    ...
    public override async Task Authorize()
    {
        ...
        var saveCreds = string.Empty;
        var useAuthCache = Config.HasCredentials;
        if (useAuthCache)
        {
            const string directory = "LNCache";
            if (!Directory.Exists(directory)) Directory.CreateDirectory(directory);

            var cacheKey = Config.Options.Login?.RemoveInvalidChars();
            saveCreds = !string.IsNullOrWhiteSpace(cacheKey) ? $"{directory}/{cacheKey}" : string.Empty;
            useAuthCache = !string.IsNullOrWhiteSpace(saveCreds);

            if (useAuthCache && File.Exists(saveCreds))
            {
                var timeDiff = DateTime.Now - File.GetLastWriteTime(saveCreds);
                if (timeDiff.TotalHours < 2)
                {
                    var savedAuth = await File.ReadAllTextAsync(saveCreds).ContinueWith(t =>
                        t.Result.Deserialize<LitnetAuthResponse>());
                    _token = savedAuth.Token;
                    DeviceId = savedAuth.DeviceID;

                    if (!string.IsNullOrWhiteSpace(_token))
                    {
                        Config.Logger.LogInformation("Litnet: auth cache hit");
                        return;
                    }
                }
                else
        ...
        {
            throw new Elib2EbookAuthException($"Не удалось авторизоваться. {data?.Error}");
        }

        if (useAuthCache)
        {
            if (data is not null) data.DeviceID = DeviceId;
    ...
    {
        var url = ApiIp.MakeRelativeUri(
            $"/v1/book/get/{bookId}?app=android&device_id={DeviceId}&user_token={token}&sign={GetSign(token)}");
        var data = await GetApiData<LitnetBookResponse>(url);
        if (!Config.HasCredentials && data!.AdultOnly)
            throw new Elib2EbookAuthException("Произведение 18+. Необходимо добавить логин и пароль.");
    ...
    {
        var url = ApiIp.MakeRelativeUri(
            $"/v1/book/contents?bookId={bookId}&app=android&device_id={DeviceId}&user_token={token}&sign={GetSign(token)}");
        return GetApiData<LitnetContentsResponse[]>(url);
    }

    private async Task<IEnumerable<LitnetChapterResponse>> GetToc(string token,
        IEnumerable<LitnetContentsResponse> contents)
    {
        var chapters = string.Join("&", contents.Select(t => $"chapter_ids[]={t.Id}"));
        var url = ApiIp.MakeRelativeUri(
            $"/v1/book/get-chapters-texts/?{chapters}&app=android&device_id={DeviceId}&sign={GetSign(token)}&user_token={token}");
    ...
    private async Task<T> GetApiData<T>(Uri uri)
    {
        ...
        if (response.StatusCode == HttpStatusCode.Forbidden)
            throw new Elib2EbookParseException("Получен бан. Попробуйте позже.");
    ...
    private async Task<Seria> GetSeria(Uri url, LitnetBookResponse book)
    {
        try
        {
            var doc = await Config.Client.SendAsync(GetDefaultMessage(url.ReplaceHost(SiteIp.Host), SystemUrl))
                .ContinueWith(t => t.Result.Content.ReadAsStream().AsHtmlDoc());
    ...
    {
        return new Author((book.AuthorName ?? SystemUrl.Host).Trim(),
            SystemUrl.MakeRelativeUri($"/ru/{book.AuthorId}"));
    }
    ...
    private IEnumerable<Author> GetCoAuthors(LitnetBookResponse book)
    {
        ...
        if (!string.IsNullOrWhiteSpace(book.CoAuthorName) && book.CoAuthorId.HasValue)
            result.Add(new Author((book.CoAuthorName ?? SystemUrl.Host).Trim(),
                SystemUrl.MakeRelativeUri($"/ru/{book.CoAuthorId.Value}")));
    ...
    {
        var message = new HttpRequestMessage(content is null ? HttpMethod.Get : HttpMethod.Post, uri);
    ...
    private async Task<List<Chapter>> FillChapters(string token, LitnetBookResponse book, string bookId)
    {
        ...
        if (contents is null || contents.Length == 0) return result;
        ...
        {
            if (!map.TryGetValue(content.Id, out var litnetChapter)) litnetChapter = new LitnetChapterResponse();
