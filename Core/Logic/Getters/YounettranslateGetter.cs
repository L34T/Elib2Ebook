...
public class YounettranslateGetter(BookGetterConfig config) : GetterBase(config)
{
    ...
    private Author GetAuthor(HtmlDocument doc)
    {
        foreach (var div in doc.QuerySelectorAll("div.col-md-3"))
        {
            ...
            if (label is not null && label.GetText().StartsWith("Автор"))
    ...
    private async Task<IEnumerable<UrlChapter>> GetToc(Uri url)
    {
        ...
        for (var i = 1;; i++)
        {
            ...
            var links = doc.QuerySelectorAll("table.catposts-table td.hidden-sm a[href]").Select(a =>
                new UrlChapter(url.MakeRelativeUri(a.Attributes["href"].Value), a.GetText())).ToList();
    ...
    private async Task<HtmlDocument> GetChapter(UrlChapter urlChapter)
    {
        ...
        return doc.QuerySelector("div.postdata-content").RemoveNodes(n => n.InnerText.Contains("ЗАПРЕЩЕНО ПУБЛИКОВАТЬ"))
            .InnerHtml.AsHtmlDoc();
    }
