...
public class LibkingGetter(BookGetterConfig config) : GetterBase(config)
{
    ...
    private HtmlNode GetTitleNode(HtmlDocument doc)
    {
        ...
        {
            if (node.Name == "#text" && string.IsNullOrWhiteSpace(node.GetText())) continue;

            if (IsTitle(node)) return node;
    ...
    private async Task<List<Chapter>> FillChapters(HtmlDocument doc, Uri baseUrl, string name)
    {
        ...
        var url = GetFirstPage(doc, baseUrl);
        while (url is not null)
        {
            ...
            fullText.Append(doc.QuerySelector("#book")
                .RemoveNodes(".navigation, .cpab, .cpab1, .load_text, #twentypersent").InnerHtml);
            i++;
        }
        ...
        if (node is not null)
            while (node is not null)
            {
                ...
                } while (node.NextSibling is not null && !IsTitle(node.NextSibling));
