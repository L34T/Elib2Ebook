...
public class WattpadGetter(BookGetterConfig config) : GetterBase(config)
{
    ...
    {
        if (url.ToString().Contains("/story/")) return GetId(url);
    ...
    public override async Task<Book> Get(Uri url)
    {
        ...
        var wattpadInfo =
            await Config.Client.GetFromJsonAsync<WattpadInfo>(SystemUrl.MakeRelativeUri($"/api/v3/stories/{storyId}"));
    ...
    {
        return !string.IsNullOrWhiteSpace(wattpadInfo.Cover)
            ? SaveImage(wattpadInfo.Cover.AsUri())
