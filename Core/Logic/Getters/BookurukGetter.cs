...
public class BookurukGetter(BookGetterConfig config) : GetterBase(config)
{
    ...
    public override async Task Authorize()
    {
        ...
        if (string.IsNullOrWhiteSpace(alert))
            ...
            throw new Elib2EbookAuthException($"Не удалось авторизоваться. {alert}");
    }
    ...
    {
        if (url.GetSegment(1) == "book") return url;
    ...
    {
        var internalId = SystemUrl
            .MakeRelativeUri(doc.QuerySelectorAll("div.content.aside-right li a")[1].Attributes["href"].Value)
            .GetSegment(2);
