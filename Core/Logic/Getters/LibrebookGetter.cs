...
public class LibrebookGetter(BookGetterConfig config) : GetterBase(config)
{
    ...
    public override async Task Authorize()
    {
        ...
        var loginPage =
            "https://1.grouple.co/internal/auth/login?siteId=6&targetUri=%2Flogin%2FcontinueSso%3FsiteId%3D6%26targetUri%3D%252F"
                .AsUri();
        ...
        if (result.Type == "success")
            ...
            throw new Elib2EbookAuthException($"Не удалось авторизоваться. {result.Text}");
    }
    ...
    public override async Task<Book> Get(Uri url)
    {
        ...
        if (chapterLink is null)
            throw new Elib2EbookParseException("Книга недоступна для чтения. Возможно, нужна авторизация.");
    ...
    private Author GetAuthor(HtmlDocument doc, Uri bookUrl)
    {
        ...
        return a is null
            ? new Author("Librebook")
            : new Author(a.GetText(), bookUrl.MakeRelativeUri(a.Attributes["href"].Value));
    }
    ...
    private async Task<IEnumerable<UrlChapter>> GetToc(Uri startUrl)
    {
        ...
        var result = doc
            ...
            .Select(a =>
                new UrlChapter(SystemUrl.MakeRelativeUri(a.Attributes["href"].Value), a.GetText().ReplaceNewLine()))
            .ToList();
