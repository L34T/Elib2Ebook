    ...
    private async Task<IEnumerable<UrlChapter>> GetToc(HtmlDocument doc, Uri url)
    {
        ...
        foreach (var span in doc.QuerySelectorAll("ul.myUL span.caret[data-id]"))
        {
            ...
            var post = await Config.Client.PostWithTriesAsync(
                SystemUrl.MakeRelativeUri("/wp-content/themes/Wuxia/template-parts/post/menu-query.php"), payload);
            var toc = await post.Content.ReadFromJsonAsync<WuxiaWorldToc[]>();
            result.AddRange(toc.Select((a, i) =>
                new UrlChapter(url.MakeRelativeUri(a.PostName), $"Глава {offset + i + 1}")));
        }
