...
public class StrokiMtsGetter(BookGetterConfig config) : GetterBase(config)
{
    ...
    public override Task Authorize()
    {
        ...
        if (string.IsNullOrWhiteSpace(token)) throw new Elib2EbookAuthException("Не указан авторизационный token.");
    ...
    public override async Task<Book> Get(Uri url)
    {
        ...
        if (Config.Options.NoChapters) return book;
        ...
        {
            if (detailBook.ContentInfo.FileType == "JPG") book.Chapters = FillChaptersFromImages(detailBook, origBooks);
        }
    ...
    private static IEnumerable<Chapter> FillChaptersFromImages(StrokiMtsBookItem book, IEnumerable<TempFile> images)
    {
        ...
        foreach (var image in images) sb.Append($"<img src=\"{image.FullName}\" />");
    ...
    private async Task<List<TempFile>> GetOriginalFiles(string bookId)
    {
        ...
        await Parallel.ForEachAsync(tuples, async (t, _) =>
        {
            ...
            var tempFile = await TempFile.Create(fileUrl.Url.AsUri(), Config.TempFolder.Path,
                fileUrl.Url.AsUri().GetFileName(), await response.Content.ReadAsStreamAsync(_));
    ...
    private async Task<List<TempFile>> GetAudio(StrokiMtsApiMultiResponse details)
    {
        ...
        var detail = details?.Items.FirstOrDefault(d => d.AudioBook is not null);
        ...
        for (var i = 0; i < fileMetas.Length; i++)
        {
            ...
            var response = await Config.Client.GetWithTriesAsync(fileUrl.Url.AsUri());
            result.Add(await TempFile.Create(fileUrl.Url.AsUri(), Config.TempFolder.Path,
                $"{i}_{fileUrl.Url.AsUri().GetFileName()}", await response.Content.ReadAsStreamAsync()));
    ...
    private Task<TempFile> GetCover(StrokiMtsBookItem book)
    {
        ...
        return !string.IsNullOrWhiteSpace(url)
            ? SaveImage(SystemUrl.MakeRelativeUri(url))
    ...
    private Author GetAuthor(StrokiMtsBookItem book)
    {
        ...
        return author is null
            ? new Author("Stroki")
            : new Author(author.Name, SystemUrl.MakeRelativeUri(author.FriendlyUrl));
    }

    private IEnumerable<Author> GetCoAuthors(StrokiMtsBookItem book)
    {
        return book.Authors.Skip(1)
            .Select(author => new Author(author.Name, SystemUrl.MakeRelativeUri(author.FriendlyUrl))).ToList();
    }

    private async Task<StrokiMtsFile[]> GetFileMeta(string id)
    {
        var json = await SendAsync<StrokiMtsApiResponse<StrokiMtsFiles>>(() =>
            GetMessage(SystemUrl.MakeRelativeUri("/api/books/files").AppendQueryParameter("bookId", id), "5.0", "5.0"));
    ...
    {
        var json = await SendAsync<StrokiMtsApiResponse<StrokiMtsFileUrl>>(() =>
            GetMessage(SystemUrl.MakeRelativeUri($"api/books/files/data/link/{file.FileId}"), "5.0", "5.0"));
        return json.Data;
    }

    private async Task<StrokiMtsApiMultiResponse> GetBook(string id)
    {
        var json = await SendAsync<StrokiMtsApiResponse<StrokiMtsApiMultiResponse>>(() =>
            GetMessage(SystemUrl.MakeRelativeUri($"/api/books/multi/{id}"), "5.53", "7.0"));
        return json.Data;
    }

    private async Task<StrokiMtsBookItem> GetSerial(string id)
    {
        var json = await SendAsync<StrokiMtsApiResponse<StrokiMtsBookItem>>(() =>
            GetMessage(SystemUrl.MakeRelativeUri($"/api/serials/episodes/{id} "), "5.53", "7.0"));
        return json.Data;
    }
    ...
    private async Task<T> SendAsync<T>(Func<HttpRequestMessage> message)
    {
        ...
        if (response.StatusCode != HttpStatusCode.OK)
            throw new Elib2EbookParseException(await response.Content.ReadAsStringAsync());
