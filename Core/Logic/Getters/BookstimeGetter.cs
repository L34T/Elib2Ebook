...
public class BookstimeGetter(BookGetterConfig config) : GetterBase(config)
{
    ...
    public override async Task Authorize()
    {
        ...
        if (post.StatusCode != HttpStatusCode.OK)
        {
            ...
            throw new Elib2EbookAuthException($"Не удалось авторизоваться. {content}");
        }
        ...
        if (string.IsNullOrWhiteSpace(response.XOctoberErrorMessage))
            ...
            throw new Elib2EbookAuthException($"Не удалось авторизоваться. {response.XOctoberErrorMessage}");
    }
