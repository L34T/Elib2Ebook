...
public static class UriExtension
{
    ...
    {
        return new Uri(self, relative);
    }

    public static Uri AppendSegment(this Uri self, string segment)
    {
        return new Uri(self.ToString().TrimEnd('/') + "/" + segment.TrimStart('/'));
    }
    ...
    {
        return string.Equals(Idn.GetAscii(self.Host).Replace("www.", ""), Idn.GetAscii(url.Host).Replace("www.", ""),
    ...
    public static bool IsSameSubDomain(this Uri self, Uri url)
    {
        ...
        return selfParts.Length >= urlParts.Length && string.Equals(
            Idn.GetAscii(string.Join(".", selfParts[^urlParts.Length..])), Idn.GetAscii(string.Join(".", urlParts)),
            StringComparison.InvariantCultureIgnoreCase);
    }
}
