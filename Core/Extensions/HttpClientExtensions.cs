...
public static class HttpClientExtensions
{
        ...
        TimeSpan errorTimeout = default)
    {
        ...
        if (response is null) throw new Elib2EbookParseException($"Не удалось получить ответ от {url}");

        if (response.StatusCode != HttpStatusCode.OK)
        {
            var errorBody = await SafeReadAsStringAsync(response.Content);
            throw new Elib2EbookFormatException(
                $"Ошибка запроса {url}. Код {(int)response.StatusCode} ({response.StatusCode}).{FormatSnippet(errorBody)}");
        }
        ...
        if (string.IsNullOrWhiteSpace(payload)) throw new Elib2EbookParseException($"Ответ {url} пустой");
        ...
        try
        {
            return JsonSerializer.Deserialize<T>(payload, new JsonSerializerOptions(JsonSerializerDefaults.Web)) ??
                   throw new Elib2EbookParseException($"Сайт вернул пустой JSON по адресу {url}");
        }
        catch (JsonException ex)
        {
            throw new Elib2EbookFormatException(
                $"Не удалось разобрать JSON ответа {url}. {ex.Message}. {FormatSnippet(payload)}", ex);
        }
    }

    public static async Task<HtmlDocument> PostHtmlDocWithTriesAsync(this HttpClient client, Uri url,
        HttpContent content, Encoding encoding = null)
    {
    ...
    private static string FormatSnippet(string value, int maxLength = 300)
    {
        ...
        if (LooksLikeHtml(trimmed)) return string.Empty;
