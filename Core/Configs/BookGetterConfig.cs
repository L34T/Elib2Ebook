...
public class BookGetterConfig : IDisposable
{
    ...
    public bool HasCredentials =>
        !string.IsNullOrWhiteSpace(Options.Login) && !string.IsNullOrWhiteSpace(Options.Password);
    ...
    {
        return Options.Settings.TryGetValue(key, out var val) ? val : defaultValue;
    }

    public bool GetBoolSetting(string key, bool defaultValue = false)
    {
        if (Options.Settings.TryGetValue(key, out var val) && bool.TryParse(val, out var res)) return res;
        return defaultValue;
    }
    ...
    public static BookGetterConfig GetDefault(Options options, ILogger logger)
    {
        ...
        return new BookGetterConfig(options, client, cookieContainer, TempFolderFactory.Create(options.TempPath),
            logger);
    }
    ...
    private static HttpClient GetClient(Options options, CookieContainer container, ILogger logger)
    {
        ...
        if (!string.IsNullOrWhiteSpace(options.Flare))
        {
            ...
            if (!string.IsNullOrEmpty(options.Proxy)) chandler.ProxyUrl = options.Proxy;
            ...
            CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (responseMessage is
                    {
                        StatusCode: HttpStatusCode.Redirect or HttpStatusCode.PermanentRedirect
                        or HttpStatusCode.MovedPermanently or HttpStatusCode.Moved,
                        Headers.Location: not null
                    })
                {
            ...
            catch (TaskCanceledException ex) when (ex.InnerException is TimeoutException)
            {
                _logger.LogInformation(
                    "Сервер не успевает ответить. Попробуйте увеличить Timeout с помощью параметра -t");
                throw;
            }
