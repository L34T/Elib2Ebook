...
public class Options
{
    ...
    [Option('f', "format", Required = true,
        HelpText = "Формат для сохранения книги. Допустимые значения: epub, fb2, cbz, json", Separator = ',')]
    ...
    [Option("additional-types", Required = false,
        HelpText = "Типы дополнительных файлов. Допустимые значения: books, audio, images", Separator = ',')]
    public IEnumerable<AdditionalTypeEnum> AdditionalTypes { get; set; }

    [Option("book-name-pattern", Required = false, HelpText = "Шаблон имени файла.",
    ...
    [Option('o', "option", Required = false, HelpText = "Дополнительные параметры провайдеров в формате Ключ=Значение")]
    public IEnumerable<string> CustomOptions { get; set; }

    public Dictionary<string, string> Settings => _settings ??= ParseCustomOptions(CustomOptions);

    public string ResourcesPath => "Patterns";

    private static Dictionary<string, string> ParseCustomOptions(IEnumerable<string> options)
    {
        ...
        foreach (var opt in options)
        {
            ...
            else if (parts.Length == 1) dict[parts[0].Trim()] = "true";
        }
    ...
    {
        return Additional && (AdditionalTypes is null || !AdditionalTypes.Any() || AdditionalTypes.Contains(type));
    }
}
