...
internal static class Program
{
    private static async Task Main(string[] args)
    {
        ...
        var parserResult = new Parser(c => { c.CaseInsensitiveEnumValues = true; }).ParseArguments<Options>(args);
            ...
            .WithParsedAsync(async options =>
            {
                ...
                foreach (var url in options.Url)
                {
                    ...
                    try
                    {
                        ...
                        if (options.Additional) await new AdditionaFileBuilder(options, logger).Build(book);

                        if (!options.SaveTemp) book.Dispose();
                    }
